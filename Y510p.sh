...
#   v1.7 :
#           -Fixed for Sierra 10.12.x
#           -using IntelBacklight (faster) instead of ACPIBacklight patches. Use IntelBacklight.kext with it.
#           -minor improvements/optimizations
#           -Added optional clover hotpatch.
...
curl -o ${tmp_d}/patches/graphics_PNLF.txt https://raw.githubusercontent.com/RehabMan/Laptop-DSDT-Patch/master/graphics/graphics_PNLF.txt >> $logFile 2>&1
...
if [ "${nfoundc}" -gt "0" ] ; then echo "     ...    '${nfoundc}' patch(es) missing! For more info see "$logFile"! Exiting!" $red; exit ; else echo "\n    >>>>   Continuing to patch DSDT\SSDT's...   <<<<    \n" >> $logFile 2>&1 ; patch_dsdt ; fi
}
...
echo "     ...    [usb] USB Fix"
# Different usb patch if El Capitan or up detected
if [[ $ProductVersion == *"10.10"* ]] ; then
...
elif [[ $ProductVersion == *"10.11"* ]] ; then
...
elif [[ $ProductVersion == *"10.12"* ]] ; then
echo "\nSierra detected...Will apply patch for same\n" >> $logFile 2>&1   #Logging Purpose Only
./tools/patchmatic ${tmp_d}/DSDT/Decompiled/DSDT.dsl ${tmp_d}/patches/usb_el_capitan.txt ${tmp_d}/DSDT/Decompiled/DSDT.dsl >> $patch_log 2>&1
fi
...
echo "     ...    [gfx] Brightness fix"
./tools/patchmatic ${tmp_d}/DSDT/Decompiled/SSDT-3.dsl ${tmp_d}/patches/graphics_PNLF.txt ${tmp_d}/DSDT/Decompiled/SSDT-3.dsl >> $patch_log 2>&1
...
echo "     ...    [gfx] Disable Nvidia card (Won't work anyway & disabling this saves significant battery)"
...
echo "     ...    [gfx] Brightness fix"
./tools/patchmatic ${tmp_d}/DSDT/Decompiled/SSDT-0.dsl ${tmp_d}/patches/graphics_PNLF.txt ${tmp_d}/DSDT/Decompiled/SSDT-0.dsl >> $patch_log 2>&1
...
echo "\n    >>>>   Compiling Started   <<<<    \n" >> $compile_log 2>&1   #Logging Purpose Only
echo "     ...    Compiling DSDT...."
...
echo "\n    >>>>   All Done...^ This column should say 0 Errors.   <<<<    \n" >> $compile_log 2>&1   #Logging Purpose Only
...
update()
{
echo "\n${cyan}${bold}Lenevo Y510p IdeaPad${normal} - (For Yosemite 10.10+ & up)"
echo ""
